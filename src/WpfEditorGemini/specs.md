...
* The DOM tree shall be built using the following C\# classes:  
  ...
  * `RefNode` (inherits from `DomNode`):  
    ...
    * **Rendering:**  
      ...
      * A modal editor shall be available for editing the link path string.  
...
* **2.3.4. Custom Hover Tooltip Renderer**
  ...
  * The editor shall allow for registering custom hover tooltip renderers for specific `System.Type`s.  
...
* **Editor Selection:**
  ...
  * A modal editor shall be available for the link path string of a `RefNode`.  
...
* **Edit Cancellation:** Pressing the Esc key shall cancel the edit. The `ValueNode`'s `Value` shall not be modified. If editing started from a `null` schematized value (which began with a C\# type default), it reverts to `null`.
...
* **Saving Changes:** Save entire DOM tree to JSON file. Un-schematized data and `RefNode`s are part of the DOM and get serialized.  
* **File Operations:**  
